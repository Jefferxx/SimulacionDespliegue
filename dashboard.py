...
if st.session_state["sidebar_open"]:
    col_s, col_m = st.columns([1, 4], gap="small")

    with col_s:
        st.markdown("""
        <div style="text-align:center; padding:6px 0 14px 0;">
            <div style="font-size:18px; font-weight:800; letter-spacing:-0.5px;">ETalent</div>
            <div style="font-size:9px; opacity:0.7; letter-spacing:1px;
        ...
        st.markdown("---")
        st.markdown('<div style="font-size:10px; font-weight:700; opacity:0.6; text-transform:uppercase; letter-spacing:1px; margin-bottom:8px;">Universidad (EIS)</div>', unsafe_allow_html=True)
        sel_eis = st.multiselect("_eis", eis_opts, default=eis_opts, key="f_eis", label_visibility="collapsed")

        st.markdown('<div style="font-size:10px; font-weight:700; opacity:0.6; text-transform:uppercase; letter-spacing:1px; margin:12px 0 8px 0;">Dimension</div>', unsafe_allow_html=True)
        sel_hoja = st.multiselect("_hoja", hoja_opts, default=hoja_opts, key="f_hoja", label_visibility="collapsed")

        st.markdown('<div style="font-size:10px; font-weight:700; opacity:0.6; text-transform:uppercase; letter-spacing:1px; margin:12px 0 8px 0;">Sub-Dimension</div>', unsafe_allow_html=True)
        sel_dim = st.multiselect("_dim", dim_opts, default=dim_opts, key="f_dim", label_visibility="collapsed")

        st.markdown("---")
        n_active = sum([len(sel_eis) < len(eis_opts), len(sel_hoja) < len(hoja_opts), len(sel_dim) < len(dim_opts)])
        filtro_txt = "PERSONALIZADO" if n_active else "GENERAL"
        st.markdown(f'<div style="font-size:9px; opacity:0.7; text-transform:uppercase; letter-spacing:0.8px; font-weight:600;">Filtros activos: {filtro_txt}</div>', unsafe_allow_html=True)
        st.markdown(f"""
        <div style="font-size:9px; opacity:0.55; line-height:1.8; margin-top:10px;">
            <b style="opacity:0.9;">Universidades:</b> {len(eis_opts)}<br>
            <b style="opacity:0.9;">Dimensiones:</b> {len(hoja_opts)}<br>
            <b style="opacity:0.9;">Sub-dimensiones:</b> {len(dim_opts)}
        </div>
        """, unsafe_allow_html=True)
    ...
    # Botón para reabrir el panel (siempre visible cuando está cerrado)
    with col_m:
        if st.button("Mostrar panel de filtros", key="btn_abrir"):
            st.session_state["sidebar_open"] = True
            st.rerun()
...
if df.empty:
    st.error("La combinacion de filtros no tiene datos. Ajusta los filtros.")
    st.stop()
...
# ──────────────────────────────────────────────────────────────
with col_m:
    st.markdown('<div style="padding: 20px 20px 0 20px;">', unsafe_allow_html=True)
    ...
    fig_heat.update_layout(
        **BASE_LAYOUT,
        ...
        xaxis=dict(side="top", tickfont=dict(size=9, color=C_RED), tickangle=-35),
        yaxis=dict(tickfont=dict(size=11, color=C_DARK), autorange="reversed"),
    )
    ...
    with c_bars:
        ...
        fig_grp.update_layout(
            **BASE_LAYOUT,
            ...
            xaxis=dict(tickangle=0, tickfont=dict(size=9, color=C_DARK)),
            legend=dict(
    ...
    with c_donut:
        ...
        for lbl, val, color in zip(d_labels, d_values, d_colors):
            ...
            <div class="legend-row">
                ...
                <span style="color:{C_GRAY}; margin-left:auto;">{val} ({pct:.0f}%)</span>
            </div>""", unsafe_allow_html=True)
    ...
    with tc_ord:
        st.markdown(
            f'<p style="color:{C_DARK};font-weight:600;font-size:12px;margin:8px 0 2px 0;">Ordenar por</p>',
            unsafe_allow_html=True,
        )
    ...
    with tc_n:
        st.markdown(
            f'<p style="color:{C_DARK};font-weight:600;font-size:12px;margin:8px 0 2px 0;">Top N</p>',
            unsafe_allow_html=True,
        )
    ...
    if criticos.empty:
        ...
        tabla = pd.DataFrame({
            ...
            "Brecha":        criticos["PROMEDIO"].apply(lambda p: f"-{(UMBRAL - p) * 100:.1f} pp"),
        ...
        st.caption(f"Mostrando {len(tabla)} indicadores unicos con promedio < {int(UMBRAL * 100)}%")
    ...
    <div style="background:{C_DARK}; padding:14px 28px; margin-top:24px;
                display:flex; justify-content:space-between; align-items:center;">
        <div style="color:{C_RED}; font-style:italic; font-size:12px;">Hacemos historia</div>
